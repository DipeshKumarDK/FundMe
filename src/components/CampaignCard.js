...
const CampaignCard = ({owner,title,description,target,deadline,amountCollected,image,handleClick,visible}) => {
  const remainingDays = daysLeft(deadline);
  return (
    <div
      className={`ml-1 mr-1 ${
        visible?._hex === "0x00" ? "" : "hidden"
      } hover:scale-105 rounded-[12px] mb-3 bg-[#1c1c24] cursor-pointer`}
      onClick={handleClick}
    >
      <img
        src={image || pic}
